...
| 60 | `948c8e0` | Docs: corrected the remote state in both handoff docs (dev is pushed through the reorg `e795630`; the `5.7.3.0` tag is on origin; only T2.2c `1c7249f` + the `5.7.3.1` tag are local) - the earlier "nothing pushed" was stale | docs only |
