...
| 57 | _next commit after `8428397`_ | **Release 5.7.3.1** (Kevin: "tag this with a bugfix version") — `VERSION` 5.7.3.0 → 5.7.3.1, rebuilt uf2, annotated tag `5.7.3.1` on this commit (local only) | builds ×3 (5.7.3.1 / 5.7.3.1 / og 1.7.3.1); flashed; `?` → 5.7.3.1; the row-56 verification is on this exact build; `i@` on → DAC0 |
