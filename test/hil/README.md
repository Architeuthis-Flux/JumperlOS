...
| `test_routing_dense.py` | 16 dense / edge-case routings pasted as state documents and checked against the schematic's copper (`fabric_v5.py`, generated by `tools/gen_fabric_v5.py`; model in `routing_check.py`): connectivity, no shorts, sent == planned, chip-status bookkeeping; two electrical proofs. `JL_ROUTING_SNAP_DIR` saves captures for `tools/routing_snapshot_diff.py` |
