...
#include "projectFiles.h" // generated by scripts/generate_projects.py (definitions: include in ONE TU)
#include "oled.h"
#include <cstring>
#include <memory>
#include <time.h>
...
//==============================================================================
// Initialize Projects - /projects/<dir>/ provisioning
//==============================================================================
// The projects sibling of the examples system (design:
// CodeDocs/DESIGN_PROJECTS_SUBSYSTEM.md section 1). The table below is the
// ready-to-paste fragment printed by scripts/generate_projects.py; the
// constants it names live in the generated src/snakes/projectFiles.h.
//
// Same struct, deliberately: a provisioned project file is exactly an
// examples entry with a different destination, and one struct means one
// meaning for knownHashes[0] across both provisioners.
using ProjectFileInfo = ExampleInfo;

static const ProjectFileInfo projectFiles[] = {
// Provisioned on BOTH boards: no INCLUDE_* board gate is needed the way
// usb_audio_mic.py needs one - project content is plain text with no hardware
// dependency, and 6 KB fits OG's 1 MB filesystem as easily as V5's 4 MB.
...
// Create every missing directory along a file's path ("/projects/555/main.py"
// -> "/projects", "/projects/555"). Keeping this derived from the table means
// a new project needs no separate directory list to fall out of sync with.
static bool ensureParentDirs( const char* filePath ) {
    String path( filePath );
    int    slash = path.indexOf( '/', 1 ); // skip the leading '/'

    while ( slash > 0 ) {
        String dir = path.substring( 0, slash );
        if ( !safeFileExists( dir.c_str( ) ) ) {
            if ( !safeMkdir( dir.c_str( ) ) ) {
                addFilesystemMessage( "ERROR: Failed to create " + dir, 196 );
                return false;
            }
        }
...
// Mirrors initializeMicroPythonExamples():
//   unforced (boot / launcher self-heal) - create what is MISSING, nothing else;
//   forced (firmware update)             - 3-way FNV-1a check per file:
//       hash == knownHashes[0]  -> already this build's default, skip
//       hash in knownHashes[1..] -> an untouched older default, overwrite in place
//       anything else            -> the user edited it: LEAVE their file alone and
//                                   drop this build's default next to it as
//                                   wiring_original.yaml / _original1 / ...
// Provisioning only ever CREATES: it never enumerates /projects and never
// deletes a directory it does not know about, so hand-made projects on the
// board coexist with the built-in ones untouched.
void initializeProjects( bool forceInitialization ) {
    ...
    bool   useOutputArea = ( globalFileManager != nullptr );
    String initAction    = forceInitialization ? "[FORCE INIT]" : "[INIT]";
    String initMsg       = initAction + " Initializing " + String( totalProjectFiles ) + " project file(s)...";
    if ( useOutputArea ) {
        globalFileManager->outputToArea( initMsg, 155 );
    } else {
        addFilesystemMessage( initMsg, 155 );
    ...
    // Same heap guard as the examples provisioner - writing raw string
    // constants out costs a File plus a copy buffer per entry.
    size_t freeHeap = rp2040.getFreeHeap( );
    if ( freeHeap < 20000 ) {
        String errorMsg = "ERROR: Low memory (" + String( freeHeap ) + " bytes), skipping project creation";
        if ( useOutputArea ) {
            globalFileManager->outputToArea( errorMsg, 196 );
        } else {
            addFilesystemMessage( errorMsg, 196 );
    ...
    for ( int i = 0; i < totalProjectFiles; i++ ) {
        if ( millis( ) - startTime > maxTime ) {
            String timeoutMsg = "TIMEOUT: Project creation aborted after 30s";
            if ( useOutputArea ) {
                globalFileManager->outputToArea( timeoutMsg, 196 );
            } else {
                addFilesystemMessage( timeoutMsg, 196 );
        ...
        const char*     canonicalPath = projectFiles[ i ].path;
        const char*     logicalName   = projectFiles[ i ].name;
        const uint32_t* knownHashes   = projectFiles[ i ].knownHashes;
        int             hashCount     = projectFiles[ i ].hashCount;
        uint32_t        currentHash   = ( hashCount > 0 ) ? knownHashes[ 0 ] : 0;

        if ( !ensureParentDirs( canonicalPath ) ) {
            continue; // its directory could not be made - skip this file, try the rest
        }

        if ( !safeFileExists( canonicalPath ) ) {
            String progressMsg = "Creating " + String( logicalName );
            if ( useOutputArea ) {
                globalFileManager->outputToArea( progressMsg, 155 );
            } else {
            ...
            if ( writeStringToFile( canonicalPath, projectFiles[ i ].content ) ) {
                filesCreated++;
            } else {
                String errorMsg = "✗ Failed to create " + String( logicalName );
                if ( useOutputArea ) {
        ...
        if ( isOldFirmwareVersion ) {
            ...
            writeStringToFile( canonicalPath, projectFiles[ i ].content );
            filesCreated++;
            delay( 10 );
            continue;
        }

        // User-edited. Don't clobber it - park this build's default in the
        // next free _original* slot beside it (there is no /lib/ carve-out
        // here: a project has no firmware-coupled library modules).
        bool      alreadyHaveDefault = false;
        const int maxVariants        = 5;
        for ( int v = 0; v < maxVariants && !alreadyHaveDefault; ++v ) {
            String   vPath = buildOriginalVariantPath( canonicalPath, v );
        ...
        if ( writeStringToFile( targetPath.c_str( ), projectFiles[ i ].content ) ) {
            filesCreated++;
        } else {
            String errorMsg = "✗ Failed to create original " + String( logicalName );
            if ( useOutputArea ) {
    ...
    String summary;
    if ( filesCreated > 0 ) {
        summary = "✓ Created " + String( filesCreated ) + " project file(s), " + String( filesSkipped ) + " unchanged";
    } else {
        summary = "✓ " + String( filesSkipped ) + " project files unchanged";
    }
    addFilesystemMessage( summary, 155 );
